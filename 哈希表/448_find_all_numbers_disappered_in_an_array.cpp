...
//     2.元素如果存在重复，那么也不会影响结果，可以自行模拟一下，如果存在重复元素，那么有个元素会占用位置
//       将元素均放在正确的位置后，再度遍历，便可以知道哪些元素不在数组中
// 时间复杂度说明
//     1.如果当前元素已经在正确的位置，则不发生替换
//     2.当存在一系列的迭代替换时，若干个元素会被放置到正确的位置，并且下次遇到这个元素时，不会再发生迭代的替换了
//       考虑所有元素都在一次迭代替换中都被重新放置，那么时间复杂度总体是O(n)，实际上每个元素只会被顶出一次，然后放置到正确的位置
//       因此，虽然有个while循环，但其每次迭代的while循环加起来的时间复杂度也还是O(n)而不是O(n^2)
//     3.综上，时间复杂度保持在O(n)
class Solution {
public:
